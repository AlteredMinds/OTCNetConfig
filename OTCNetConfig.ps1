...
function NetSettings()
{
    ...
    $adapterStatistics | Select-Object Name, 
						     ...
						     @{Name='Broad<-'; Expression={$_.SentBroadcastPackets}} | Format-Table -AutoSize -Property Name, Uni<-, Multi<-, Broad<-, Out,
						     @{Label="Packets"; Expression={"   |  "}}, In, Uni->, Multi->, Broad->
...
function MapStudata()
{
    ...
    $FQ_Letter = $studataLetter + ':'
    $FQ_username = "otc\" + $username
    New-SmbMapping -LocalPath $FQ_Letter -RemotePath \\fs-studata.otc.edu\studata\iti -user $FQ_username -password $password -Persistent $true
...
for ($i = 1; ($i * $exit) -lt 1; $i++)
{
    ...
    Write-Host " 5) Studata Setup" -ForegroundColor Gray
    Write-Host " 6) About" -ForegroundColor Gray
    Write-Host " 7) Exit" -ForegroundColor Gray
    #Write-Host " ;) Install Backdoor" -ForegroundColor Gray #Easter Egg#
    Write-Host " *************************" -ForegroundColor DarkGray
    $selection = Read-Host -Prompt " > Select an option"
	  ...
	  ###If selection is 7 then go to website###
